    ...
    deleteAccount() {
        ...
        return defer(() => from(Promise.all([Promise.resolve(this.workoutsAuthToken()), Promise.resolve(this.userWorkoutsAuthToken())]))).pipe(
            switchMap(([workoutsAuthToken, userWorkoutsAuthToken]) => {
                ...
                // Firebase deleteUser requires a recent credential sign-in. Persisted sessions
                // (open the app the next day) almost always fail with requires-recent-login.
                // Wipe Upstash only after Firebase succeeds — otherwise a returning user who
                // taps Delete account loses every owned workout and history while still signed in.
                //
                // Capture tokens first: deleteUser signs the client out, but the JWT stays valid
                // until exp, so the same-session Upstash cleanup can still authorize.
                // List with the *captured* workouts token — not a live auth.currentUser lookup.
                // Owner-stamped workouts are filtered out of unauthenticated GETs.
                return this.authService.deleteCurrentUser().pipe(
                    switchMap(() => this.workoutsService.getWorkouts(undefined, workoutsAuthToken)),
