...
export class WorkoutsService {
    ...
    private readOptions(authToken: string | Promise<string | null> | null = this.authTokenProvider()): Observable<{ headers?: { Authorization: string } }> {
        const token = authToken;
        if (!token) {
            return of({});
        }
    ...
    getWorkouts(search?: string): Observable<TabataWorkout[]> {
        ...
        return this.readOptions().pipe(switchMap((options) => this.http.get<TabataWorkout[]>(this.apiUrl('/api/workouts'), { ...options, params })));
    }

    /** GET a single workout by id (if your proxy supports it). */
    getWorkoutById(id: string): Observable<TabataWorkout | null> {
        return this.readOptions().pipe(
